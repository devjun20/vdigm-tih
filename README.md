...
### TROUBLESHOOTING: 악의적 요청으로부터 API 비용 방어

**문제:** LLM API 호출은 건당 비용이 발생합니다. Rate Limit은 시간당 누적 토큰 비용을 기준으로 판정하는데, 판정이 완료되기 전에 악의적 사용자가 대량 요청을 동시에 보내면 비용이 통제 불가능하게 폭발하는 구조적 허점이 존재했습니다.

**해결:**

1. **비용 기반 Rate Limit:** 1시간 누적 토큰 비용이 임계치를 초과하면 해당 사용자를 1시간 차단
2. **엔드포인트 Rate Limit:** 비용 판정 지연을 악용한 대량 요청을 원천 차단하기 위해, LLM 호출 엔드포인트에 요청 수 제한을 별도로 적용
3. **이중 방어:** Cloud Armor(인프라 레벨) + FastAPI 미들웨어(서비스 레벨) 양쪽에서 제한하여 단일 지점 우회 방지
4. **공격 벡터 선제 탐색:** 코드 리뷰 과정에서 IP 변경을 통한 Rate Limit 우회 등의 공격 루트를 사전에 식별하고, Cloud Armor 규칙을 강화하여 선제 대응
...
### TROUBLESHOOTING: GPU 추론 서버 병목 지점 탐색

**배경:** 자체 GPU(T4)에서 GPT-SoVITS를 서빙할 때, 동시 요청이 늘어나면 어느 지점에서 서비스가 불가능해지는지 확인할 필요가 있었습니다.

**테스트:** 10초 간격으로 10건의 TTS 요청을 지속적으로 전송하는 시나리오를 구성하고, 직접 제작한 테스트 UI를 통해 응답 시간을 실시간 모니터링했습니다.

![GPU 병목 테스트 결과](assets/simulator/GPU%20최적화%20후.png)

**발견:** 일정 시점 이후 하나의 요청이 지연되면 후속 요청이 연쇄적으로 밀리며 응답 시간이 급격히 증가했습니다. 병목 분석 결과 CPU와 VRAM은 여유가 있었으나, **GPU의 추론 처리량(Throughput) 자체가 한계**였습니다.

**판단:**

- 단일 GPU 스케일업이 아닌 **다중 GPU 수평 확장**이 필요한 구조
- TTS 모델은 대형 모델이 아니라 VRAM 요구량이 낮고 NVLink가 불필요하므로, 운영 환경이 허락한다면 **소비자용 GPU(RTX 3090/5090)가 비용 대비 효율적**일 수 있다고 판단 (단, 발열·안정성·보증 등 운영 리스크는 별도 검증 필요)
- 현실적으로 GPU 확보가 불안정했기 때문에 ElevenLabs SaaS 전환을 우선하되, Facade 패턴으로 자체 호스팅 복귀가 가능하도록 설계
